...
-- =====================================================
-- INSTRUCTIONS:
-- 1. In phpMyAdmin, DROP your existing database if it exists
-- 2. Create a new database (e.g., safemati)
-- 3. Select the database
-- 4. Import this entire file
-- =====================================================
...
DROP TABLE IF EXISTS `disaster_guides`;
...
CREATE TABLE `alerts` (
  ...
  CONSTRAINT `fk_alert_issued_by` FOREIGN KEY (`issued_by`) REFERENCES `users` (`user_id`) ON DELETE SET NULL ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE `incident_reports` (
  ...
  CONSTRAINT `fk_incident_user` FOREIGN KEY (`user_id`) REFERENCES `users` (`user_id`) ON DELETE SET NULL ON UPDATE CASCADE,
  CONSTRAINT `fk_incident_responder` FOREIGN KEY (`responded_by`) REFERENCES `users` (`user_id`) ON DELETE SET NULL ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
